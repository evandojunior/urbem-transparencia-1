...
//		Sessao::set('uf_sigla', $uf->getSigla());
	...
	public static function listaBalanceteDespesaOrgaoToJson() {
        $search = new Search();
        return BalanceteDespesaBO::filter($search, 'getBalanceteDespesaOrgao', []);
    }
    ...
    public static function listaBalanceteReceitaMesJson() {
        $search = new Search();
        $ultimoMesImportacao = getMesData((ImportacaoBO::getUltimo()->data_limite_dado), 'US');
        $mes = isset($_REQUEST['mes']) ? $_REQUEST['mes'] : getMesExtenso($ultimoMesImportacao);
        $balanceteReceitaDTO = [];
        ...
        return [$balanceteReceitaDTO, $totais->fetch(), $mes];
    }
    ...
    public static function listaCargoJson() {
        $search = new Search();
        $search->setMax(999999999999);

        $cargosDTO = CargoBO::filter($search, 'getCargo', ['competencia_json' => true]);

        $competencia = RemuneracaoBO::getLastRemuneracao();


	    return [$cargosDTO->getObj(), !empty($competencia) ? $competencia[0]['mes_ano'] : null];
    }
    ...
    public static function listaServidorJson() {
        $search = new Search();
        $search->setMax(999999999999);

        $servidoresDTO = ServidorBO::filter($search, 'getServidor', $_REQUEST);
	    return $servidoresDTO->getObj();
    }
    ...
    public static function listaRemuneracaoJson() {
        $search = new Search();
        $search->setMax(9999999999);

        $remuneracoesDTO = RemuneracaoBO::filter($search, 'getRemuneracao', $_REQUEST);
        return $remuneracoesDTO->getObj();
    }
